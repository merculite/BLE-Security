...
def readreq(BT_conn, handle):
	read = "024C000700030004000a" + handle
...
def remote_name(s):
	raw = "0119040a8a34ec1a334002000000"
	raw = bytearray.fromhex(raw)
	s.send(raw)
	time.sleep(.1)

def info_ext_feature(s):
	raw = "020b000a00060001000a0102000200"
	raw = bytearray.fromhex(raw)
	s.send(raw)
	time.sleep(.1)

def info_fixed_chan(s):
	raw = "020b000a00060001000a0202000300"
	raw = bytearray.fromhex(raw)
	s.send(raw)
	time.sleep(.1)
